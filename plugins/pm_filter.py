...
async def next_page(bot, query):
    ...
    if int(req) not in [query.from_user.id, 0]:
        return await query.answer("You have no Reason to Tap This 😄", show_alert=True)
    try:
        offset = int(offset)
    except:
        offset = 0
...
async def cb_handler(client: Client, query: CallbackQuery):
        ...
        if chat_type == enums.ChatType.PRIVATE:
            ...
            if grpid is not None:
                ...
                try:
                    ...
                    return await query.answer('Piracy Is Crime baby 🌝')
            else:
    ...
    if query.data.startswith("file"):
        ...
        buttons = [[
            InlineKeyboardButton('➕ ᴀᴅᴅ ᴍᴇ ᴛᴏ ʏᴏᴜʀ ɢʀᴏᴜᴘs ʙᴀʙʏ ➕', url=f'http://t.me/{temp.U_NAME}?startgroup=true')
        ], [
            InlineKeyboardButton('🔍 sᴇᴀʀᴄʜ ᴍᴏᴠɪᴇs', switch_inline_query_current_chat=''),
            InlineKeyboardButton('🤖 ᴜᴘᴅᴀᴛᴇs', url='https://t.me/moveandroll')
        ], [
        ...
        await query.answer('Piracy Is Crime baby 🌝')
...
async def auto_filter(client, msg, spoll=False):
    ...
    if offset != "":
        ...
        btn.append(
            [InlineKeyboardButton(text=f"📑 1/{math.ceil(int(total_results) / 15)}", callback_data="pages"),
    ...
    if imdb:
        ...
        cap =f"<b>Title : </b>{search}\n<b>User : </b>{msg.from_user.mention}\n<b>ID : </b>{q.from_user.id}\n<b>Results : </b>{total_results}\n\n<b>© : {msg.chat.title}</b>"
